...
from util.visualization import WriterTensorboardX
...
class BaseTrainer:
    ...
    def __init__(self, model, loss, optimizer, resume, config, train_logger=None):
        ...
        # configur_valid_epochation to monitor model performance and save best
        ...
        save_json(config, config_save_path, n_indent=4, if_sort_key=False)
    ...
    def _prepare_device(self, n_gpu_use):
        ...
        if n_gpu_use > 0 and n_gpu == 0:
            self.logger.warning("Warning: No GPU available on this machine; training will be performed on CPU.")
            n_gpu_use = 0
        if n_gpu_use > n_gpu:
            msg = "Warning: n. GPU\'s configured to use: {}, but only {} are available.".format(n_gpu_use, n_gpu)
